...
public class ComponenteMapa extends JComponent implements ObservadorSimuladorTrafico {
	...
	private void drawMap(Graphics g) {
		...
		for (Carretera e : this.mapa.getCarreteras()) {
			Punto p1 = posicionesDeNodos.get(e.getSource().getId());
			Punto p2 = posicionesDeNodos.get(e.getTarget().getId());
			...
			for (Vehiculo d : e.getVehicles()) {
				...
				Color dotColor = d.getTiempoDeAveria() > 0.5 ? Color.RED : Color.MAGENTA;
	...
	private boolean hasGreen(Carretera e) {
		/*for (CarreteraEntrante r : e.getTarget().getCarreteras()) {
...
//		return false;
	...
	CarreteraEntrante r = e.getTarget().getCarreteraEntranteVerde();
	return r.getRoad().equals(e);
